...
```{r DJresid, fig.cap="Residuals from forecasting the Dow Jones Index using the naïve method.", warning=FALSE, dependson="DJautoplot"}
res <- residuals(naive(dj2))
autoplot(res) + xlab("Day") + ylab("") +
  ggtitle("Residuals from naïve method")
```

```{r DJhist, fig.cap="Histogram of the residuals from the naïve method applied to the Dow Jones Index. The left tail is a little too long for a normal distribution.", warning=FALSE, message=FALSE, dependson="DJresid"}
gghistogram(res) + ggtitle("Histogram of residuals")
```

```{r DJacf, fig.cap="ACF of the residuals from the naïve method applied to the Dow Jones Index. The lack of correlation suggesting the forecasts are good.", fig.asp=0.35, dependson="DJresid"}
...
```{r checkresiduals, warning=FALSE, dependson="DJautoplot"}
...
The `window` function introduced in Chapter \@ref(graphics) is very useful when extracting a portion of a time series, such as we need when creating training and test sets. In the `window` function, we specify the start and/or end of the portion of time series required using time values. For example,
...
```{r djaccuracytable, echo=FALSE, dependson="djfc"}
dj3 <- window(dj, start=251)
tab <- matrix(NA,ncol=4,nrow=3)
...
As discussed in Section \@ref(perspective), a prediction interval gives an interval within which we expect $y_{t}$ to lie with a specified probability. For example, assuming that the forecast errors are normally distributed, a 95% prediction interval for the $h$-step forecast is
$$
...
```{r djpi, echo=FALSE, dependson="DJresid"}
...
```{r djforecasts, echo=TRUE, dependson="DJautoplot"}
naive(dj2)
```
...
```{r djforecasts2, echo=TRUE, dependson="DJautoplot"}
autoplot(naive(dj2))
```
...
```{r djforecastsboot, echo=TRUE, dependson="DJautoplot"}
...
If the first argument is of class `ts`, it returns forecasts from the automatic ETS algorithm discussed in Chapter \@ref(expsmooth).
...
7. For your retail time series (from Exercise 3 in Section \@ref(graphics-exercises)):
...
## Further reading {#toolbox-reading}
