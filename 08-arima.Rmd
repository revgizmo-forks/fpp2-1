...
This is the model behind the drift method discussed in Section \@ref(simple-methods).
...
The following R code was used to select a model automatically. (We use the argument `seasonal=FALSE` for now, but we will consider seasonal ARIMA models in Section \@ref(seasonal-arima) )
...
```
The returned objects can then be passed into `tsCV`. Let's consider ARIMA models and ETS models for the `air` data as introduced in Section \@ref(trend) where, `air <- window(ausair, start=1990)`.
```{r include=FALSE}
...
5. For your retail data (from Exercise 3 in Section \@ref(graphics-exercises)), find the appropriate order of differencing (after transformation if necessary) to obtain stationary data.
