...
```
The returned objects can then be passed into `tsCV`. Let's consider ARIMA models and ETS models for the `air` data as introduced in Section \@ref(holt) where, `air <- window(ausair, start=1990)`.
```{r include=FALSE}
